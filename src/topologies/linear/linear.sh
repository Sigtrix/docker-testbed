...
docker run -d --name r1 --network c1-r1 --ip 10.0.1.2 --privileged linear-image
docker network connect --ip 10.0.2.2 r1-r2 r1
docker run -d --name r2 --network r2-s1 --ip 10.0.3.2 --privileged linear-image
docker network connect --ip 10.0.2.3 r1-r2 r2
docker run -d --name c1 --network c1-r1 --ip 10.0.1.4 --privileged linear-image
docker run -d --name s1 --network r2-s1 --ip 10.0.3.4 --privileged linear-image
